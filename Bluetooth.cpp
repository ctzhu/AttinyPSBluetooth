...
Bluetooth::Bluetooth(uint32_t baudRate, uint8_t softwareTXPin)
{
	_useSoftwareSerial = true;
	if (_useSoftwareSerial)
	{
		_serialStream = new SendOnlySoftwareSerial(softwareTXPin);
		((SendOnlySoftwareSerial*)_serialStream)->begin(baudRate);
	}
	// else
	// {
